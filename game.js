...
function gameLoop(){
  ...
  if(lives <= 0 || !gameRunning){
    ...
    drawScore();
    drawLives();    
    drawGameOver("Ohh SpongeBob...why??!");
    drawBottomHelper();
    return false;
  }
...
function helperHandler(){
  if(aliveInvaders == armyColumns* armyRows){
    drawBottomMessage("press SPACE to zap laser", 125);
  }  else
  if(hasLifeDecreased){
...
function drawBottomHelper(){
  drawBottomMessage("Press enter to begin mission",150)
}
...
function drawGameOver(message){
  drawBlinker(function(){ drawScreen__line1("mission failed ") },function(){ drawScreen__line2(message) });
}
...
function drawBlinker(func1, func2){
  ...
  function drawStartScreen(){
    drawBlinker(function(){ drawScreen__line1("Space Invaders") },function(){ drawScreen__line2("press enter to begin mission") });
  }
